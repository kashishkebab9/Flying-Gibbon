...
def flight_control(x0, xf, h=0.01, T_max=6.0, config_file="config.yaml"):
    ...
    opti.subject_to(arm_end_x == xf[0])
    opti.subject_to(arm_end_y == xf[1])
    ...
    objective += (arm_end_x - xf[0]) ** 2 
    objective += (arm_end_y - xf[1]) ** 2 
    objective +=  (y_dot[N]) ** 2 
    # for k in range(N):
...
def simulate_projectile(t_release, config_file="config.yaml", h=0.01):
    ...
    initial_state = np.array([x_release, y_release, theta, v_x, v_y, phi_dot_release, alpha_release, 0])
