...
# ===== Simulation / RandUP =====
M, N, dt = 5000, 100, 0.05         # particles, steps, timestep
...
# ==== Analysis on this output ==== 
# - We need to, for each of the "bins" of our torus, determine the percentage a theta value is covered (area of phi
# for a given theta)
...
points = np.zeros((NUM_THETA_BINS* NUM_PHI_BINS, 3))
for theta_idx in range(NUM_THETA_BINS):
    for phi_idx in range(NUM_PHI_BINS):
        theta = 2*np.pi * theta_idx / NUM_THETA_BINS     # radians
        phi = 2*np.pi * phi_idx   / NUM_PHI_BINS
        point = torus_embed(theta, phi)
        index = theta_idx * NUM_PHI_BINS + phi_idx
        points[index, :] = point
        # we need to see if the corresponding point is within our hull or not
output = in_hull(points, P)
# Reshape output into a 2D grid: [theta_idx, phi_idx]
coverage_grid = output.reshape((NUM_THETA_BINS, NUM_PHI_BINS))

# Compute percentage of phi bins covered for each theta
phi_coverage_percent = np.mean(coverage_grid, axis=1) * 100

# θ values for plotting (in radians)
theta_vals = np.linspace(0, 2*np.pi, NUM_THETA_BINS, endpoint=False)

# Plot
plt.figure(figsize=(10, 4))
plt.plot(theta_vals, phi_coverage_percent, color='royalblue')
plt.xlabel("θ (radians)")
plt.ylabel("Covered φ bins (%)")
plt.title("Percentage of φ-points Covered by Convex Hull for Each θ")
plt.grid(True)
plt.tight_layout()
plt.show()
